...
class Agent:
        ...
        if self.model and hasattr(self.model, 'model'):
            ...
            # Qwen models
                ...
                if 'qwen3' in model_name:
                    return 500000
                elif 'qwen-max' in model_name or 'qwen2.5' in model_name:
                    return 131072
    ...
    def cancel(self):
        """Signal the agent to stop execution. Cancels ongoing LLM calls and tool execution."""
        self._cancel_event.set()
        logger.info("[Agent] Cancel event set")

    def reset_cancel(self):
        """Reset the cancel event so the agent can accept new requests."""
        self._cancel_event.clear()
        logger.info("[Agent] Cancel event cleared")
