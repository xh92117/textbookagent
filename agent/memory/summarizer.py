...
SUMMARIZE_SYSTEM_PROMPT = """你是每日记忆整理器。把对话压缩成当天值得保留的事件记录。

## 记录标准

- 只记录稳定、有后续价值的信息：用户决定、偏好、项目进展、待办、关键结论、重要错误。
- 合并同一事件的多轮对话，每条一行，以 "- " 开头。
- 保留必要上下文，但不要复述完整聊天过程。

## 不要记录
- 问候、闲聊、临时调试噪声、重复日志、无结果的尝试。
- API key、token、密码、隐私凭据等敏感信息。
- 模型自己的猜测、未确认事实、无依据推断。

如果没有记录价值，直接输出：无。"""

SUMMARIZE_USER_PROMPT = """请归纳以下对话的每日记忆：
...
DREAM_SYSTEM_PROMPT = """你是长期记忆蒸馏器，负责把近期每日记忆合并进 MEMORY.md。

你将收到两份材料：
1. 当前长期记忆 MEMORY.md
2. 近期每日记忆

MEMORY.md 会影响后续对话，必须高密度、低噪声、可长期使用。

## 严格边界
- 只能基于提供材料整理；不得编造、推测或添加材料中不存在的信息。
- 不写入临时状态、过程流水、低价值错误、重复日志、短期计划。
- 不写入敏感信息，例如 API key、token、密码。
- 冲突时以较新的每日记忆为准，但要保守表述。
...
输出完整 MEMORY.md 内容：
- 合并近义条目，删除已被覆盖的旧表达。
- 新增稳定偏好、长期目标、项目规则、重要决策、可复用经验。
- 每条一行，以 "- " 开头；可用 "## 标题" 分组。
- 控制在 50 条以内，每条尽量一句话。

### Part 2: 蒸馏日志 [DREAM]

用简短文字说明本次合并、删除、冲突处理和新增洞察。
...
class MemoryFlushManager:
    ...
    def get_main_memory_file(self, user_id: Optional[str] = None) -> Path:
        """Get main memory file path under the system memory directory."""
        if user_id:
            user_dir = self.memory_dir / "users" / user_id
            user_dir.mkdir(parents=True, exist_ok=True)
...
def create_memory_files_if_needed(workspace_dir: Path, user_id: Optional[str] = None):
    ...
    # Create main MEMORY.md in system memory (always needed for bootstrap).
    if user_id:
    ...
    if not main_memory.exists():
        legacy_main = Path(workspace_dir) / "MEMORY.md"
        if not user_id and legacy_main.exists() and legacy_main.is_file():
            main_memory.write_text(legacy_main.read_text(encoding="utf-8"), encoding="utf-8")
        else:
