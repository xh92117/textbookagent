...
class MemoryStorage:
    ...
    def delete_windows_style_paths(self):
        """Delete legacy index rows whose relative paths contain backslashes."""
        self.conn.execute("DELETE FROM chunks WHERE instr(path, '\\') > 0")
        self.conn.execute("DELETE FROM files WHERE instr(path, '\\') > 0")
        self._rebuild_fts_if_available()
        self.conn.commit()

    def _rebuild_fts_if_available(self):
        if not self.fts5_available:
            return
        try:
