...
class ConversationStore:
    ...
    def has_recent_text_message(
        ...
        """
        Return True when an equivalent visible text message already exists.

        This is used by UI-side save endpoints that may be called alongside
        the streaming agent persistence path. It keeps the unified SQLite
        history complete without duplicating the same browser-visible turn.
        """
        text = (text or "").strip()
        ...
        with self._lock:
            ...
            try:
                rows = conn.execute(
                    ...
                    SELECT content
                    FROM messages
                    WHERE session_id = ? AND role = ? AND created_at >= ?
                    ORDER BY seq DESC
                    LIMIT 30
                    """,
                    (session_id, role, cutoff),
                ).fetchall()
            finally:
                conn.close()
        for (raw_content,) in rows:
            try:
                ...
                content = raw_content
            if _extract_display_text(content).strip() == text:
                return True
        return False
