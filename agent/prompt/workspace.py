...
### 📝 写下来 - 不要"记在心里"！
- **记忆是有限的** - 只记录会影响后续工作的事实、偏好、规则和结论
- "记在心里"不会在会话重启后保留，重要信息应写入文件
- 当有人说"记住这个" → 更新 `MEMORY.md` 或 `memory/YYYY-MM-DD.md`
- 当你学到可迁移的教训 → 更新 RULE.md 或相关技能
- 当你遇到可复现、会影响后续工作的错误 → 记录成简短教训；临时网络失败、404、重复尝试不要沉淀
...
知识库只保存可复用、长期有价值的结构化知识。写入前先判断价值和边界，避免把临时过程、低质量结果、重复日志塞进知识库。

可以写入：
- 用户明确要求保存到知识库的资料或结论
- 上传、检索、整理得到的教材相关资料，且后续章节会作为证据引用
- 稳定的概念、实体、方法论、规则、术语表、案例和经过验证的结论
- 项目长期复用的设计决策、实现规范或排错经验

不要写入：
- 闲聊、一次性计划、尚未验证的猜测、低质量搜索结果
- 临时失败记录、404/超时/限流等普通网络噪声
- 与当前教材或用户长期目标无关的资料
- 敏感信息、密钥、账号、隐私数据

回复中可以简短说明已保存的内容；若价值不确定，优先完成任务，不要为了写入而额外消耗轮次。
...
4. **风格要求**：专业严谨、简洁清晰，整体控制在 100 字以内，不使用 emoji
