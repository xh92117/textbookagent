...
class AgentInitializer:
    ...
    def _load_tools(self, workspace_root: str, memory_manager, memory_tools: List, session_id: Optional[str] = None):
        ...
        for tool_name in tool_manager.tool_classes.keys():
            try:
                # Prefer skill-based search (web_fetch + multi-search-engine)
                # over the legacy Bocha web_search wrapper. Keeping web_fetch
                # available lets the skill choose Google/Bing/Baidu/etc.
                if tool_name == "web_search":
                    logger.debug("[AgentInitializer] WebSearch skipped - use multi-search-engine skill via web_fetch")
                    continue
                ...
                if tool:
                    ...
                    if tool_name in ['read', 'write', 'edit', 'bash', 'grep', 'find', 'ls', 'web_fetch', 'knowledge_capture', 'textbook_outline', 'textbook_chapter', 'send', 'browser', 'memory_graph_context', 'memory_graph_status']:
