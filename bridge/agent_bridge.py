...
class AgentBridge:
    ...
    def agent_reply(self, query: str, context: Context = None, 
        ...
        try:
            ...
            process_id = f"{session_id or 'default'}_{int(time.time() * 1000)}"
            ...
            try:
                ...
                process_recorder = RealtimeMemoryRecorder(workspace_root)
            ...
            def process_event_handler(event):
                ...
                try:
                    ...
                    if event_type == "tool_execution_start":
                        ...
                        process_recorder.update_process(process_id, "tool_end", f"{tool_name}: {status}", tool=tool_name, status=status)
                    elif event_type in ("pipeline_start", "phase_start", "phase_complete", "pipeline_complete"):
                        process_recorder.update_process(process_id, event_type, data.get("phase") or data.get("result_summary") or event_type)
                except Exception:
                    pass
            ...
            try:
                ...
                response = agent.run_stream(
                    user_message=query,
                    on_event=process_event_handler,
            ...
            try:
                if 'process_recorder' in locals() and process_recorder:
    ...
    def _refresh_conditional_tools(agent):
        """
        Remove legacy conditional web_search.

        Textbook agents now use the multi-search-engine skill through web_fetch
        instead of Bocha/LinkAI web_search. Keeping this method as a cleanup hook
        prevents old long-running agents from re-injecting web_search after env
        refreshes.
        """
        try:
            ...
            if has_tool:
                ...
                logger.info("[AgentBridge] legacy web_search tool removed; use multi-search-engine skill via web_fetch")
        except Exception as e:
